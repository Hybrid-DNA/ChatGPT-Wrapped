...
def build_wrapped_jpg(title: str,
    ...
    if not time_over_time.empty and "time" in time_over_time.columns:
        fig.add_trace(
            go.Scatter(
                x=time_over_time["time"],
                y=time_over_time["duration_minutes"],
                mode="lines",
                name="Active minutes",
                line=dict(color=DATA_COLORS[1], width=2, dash="dash"),
                fill="tozeroy",
                fillcolor="rgba(129,196,179,0.18)",
            ),
            row=1,
            col=2,
        )
    ...
    top_time = time_cat.head(6)
    if not top_time.empty:
        fig.add_trace(
            go.Bar(
                x=top_time["duration_minutes"],
                y=top_time["category"],
                orientation="h",
                marker=dict(color=DATA_COLORS[::-1]),
                text=top_time["duration_minutes"].round().astype(int),
                textposition="auto",
                name="Minutes",
                opacity=0.65,
            ),
